...
// }
  ...
  struct OpticalFlow : public FrameProcessor {
    ...
    virtual json process( int f )
    {
      ...
      int count = 0;
      const int gutter = 5;
      for( auto r = gutter; r < (scaledFlow.rows-gutter); ++r ) {
        for( auto c = gutter; c < (scaledFlow.cols-gutter); ++c ) {

          if( !scaledMask.empty() && scaledMask.at<uchar>(r,c) == 0 ) continue;

          CostFunction* cost_function = new AutoDiffCostFunction<OpticalFlowFunctor, 2, 1, 1, 2, 2>(new OpticalFlowFunctor( scaledFlow.at<Vec2f>(r,c), cv::Point(c,r) ));
      ...
      LOG(INFO) << "Used " << count << " of " << (scaledFlow.rows-2*gutter)*(scaledFlow.cols-2*gutter) << " flow points";
